...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError('Failed to sign in: ' + (err.message || 'Check your credentials'));
    } finally {
      setLoading(false);
    }
  }
    ...
    try {
      ...
      if (err.message && err.message.includes('canceled')) {
        ...
        setError('Sign-in popup was blocked. Please allow popups for this site and try again.');
                 ...
                 err.message && err.message.includes('Try using Google Redirect')) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        {error && <div className="bg-destructive/10 text-destructive px-4 py-3 rounded relative" role="alert">{error}</div>}
